...
app.post("/signin", (req, res) => {
  ...
  let encryptedPassword = encrypter.update(password, "utf8", "hex");
  encryptedPassword += encrypter.final("hex");
      ...
      if (!err) {
        if (result.rows[0].count == 0) {
          let insertQuery = `insert into signup(firstname, lastname, email, password)
                              values('${user.fname}', '${user.lname}', '${user.email}', '${encryptedPassword}') `;
      ...
      if (!err) {
        if (result.rows[0].count == 1) {
          ...
          pool.query(passQuery, (err, result1) => {
            ...
            let decryptedMsg = decrypter.update(passdb, "hex", "utf8");
